...
Запуск: python mock_data.py (из корня back/ или python back/mock_data.py)
...
def create_test_tracks_with_lessons():
    ...
    tracks_data = [
        ...
        {
            "title": "Интерактивные веб-лекции",
            "description": "Демонстрация лекций на основе HTML-файлов: полная свобода вёрстки, интерактивные элементы, анимации и встроенный JavaScript.",
            "order": 5,
        },
    ]
    ...
    for track in created_tracks:
        ...
        elif track.title == "Интерактивные веб-лекции":
            lessons.append(add_lecture("Демо: интерактивная HTML-лекция", [
                {"type": "text", "content": (
                    "<h2>Веб-лекции в платформе Kavnt</h2>"
                    "<p>Этот урок демонстрирует новый тип блока — <strong>веб-файл</strong>. "
                    "Вы можете загружать собственные HTML-файлы с CSS и JavaScript, "
                    "и они будут отображаться внутри лекции как полноценные интерактивные страницы.</p>"
                    "<h3>Возможности</h3>"
                    "<ul>"
                    "<li>Полная свобода вёрстки — любой HTML, CSS, JS</li>"
                    "<li>Интерактивные элементы и анимации</li>"
                    "<li>Адаптивный дизайн для мобильных устройств</li>"
                    "</ul>"
                )},
                {"type": "web_file", "url": "/web-lection-files/index.html", "title": "Пример веб-лекции"},
            ]))
            lessons.append(add_lecture("Комбинированная лекция: текст + веб-файл", [
                {"type": "text", "content": (
                    "<h2>Комбинирование блоков</h2>"
                    "<p>Веб-файл можно комбинировать с другими блоками: текстом, вопросами, кодом и видео. "
                    "Это позволяет создавать богатые образовательные материалы.</p>"
                )},
                {"type": "web_file", "url": "/web-lection-files/index.html", "title": "Интерактивный пример"},
                _qblock("q-web", "Что позволяет блок 'веб-файл'?", "Выберите все верные варианты:",
                    [QuestionChoiceEmbed(id="c1", text="Использовать HTML/CSS/JS в лекции", is_correct=True),
                     QuestionChoiceEmbed(id="c2", text="Добавлять интерактивные элементы", is_correct=True),
                     QuestionChoiceEmbed(id="c3", text="Заменить все другие типы блоков", is_correct=False)],
                    multiple=True, hints=["Блок веб-файл — это дополнительный инструмент, а не замена остальным блокам."]),
            ]))
...
def create_web_lecture_track(users, groups):
    """Создает демонстрационный трек с веб-файловой лекцией."""
    print("\n[*] Создание трека с интерактивными веб-лекциями...")

    teacher = next((u for u in users if getattr(u, "username", "") == "teacher2"), None)
    if not teacher:
        print("  [WARN] Учитель teacher2 не найден, трек будет создан без автора.")
        teacher_id = ""
        teacher_group_ids = []
    else:
        teacher_id = str(teacher.id)
    ...
    track_title = "Интерактивные веб-лекции"
    Track.objects(title=track_title).delete()
    track = Track(
        title=track_title,
        description=(
            "Демонстрационный трек с интерактивными HTML-уроками, "
            "загружаемыми через блок веб-файла."
        ),
        order=3,
        lessons=[],
        visible_group_ids=teacher_group_ids,
    ...
    # Лекция с web_file блоком
    pid = _stable_public_id(f"lecture:{track_title}:Демо: интерактивный HTML-урок")
    Lecture.objects(public_id=pid).delete()
    lec = Lecture(
        title="Демо: интерактивный HTML-урок",
        track_id=track_id,
        content="",
        blocks=[
            {
                ...
                "content": (
                    "## Интерактивные веб-лекции\n"
                    "Этот урок демонстрирует загрузку интерактивного HTML-контента "
                    "через блок **веб-файла**."
                ),
            },
            {
                "type": "web_file",
                "url": "/web-lection-files/151/index.html",
    ...
    lec.public_id = pid
    lec.save()
    lessons.append(LessonRef(id=str(lec.id), type="lecture", title=lec.title, order=order))
    order += 1

    # Ещё одна лекция с текстовым блоком для контекста
    pid2 = _stable_public_id(f"lecture:{track_title}:Что такое веб-файлы")
    Lecture.objects(public_id=pid2).delete()
    lec2 = Lecture(
        title="Что такое веб-файлы",
        track_id=track_id,
        content="",
        blocks=[
            {
                ...
                "content": (
                    "### Блок веб-файла\n"
                    "Блок `web_file` позволяет встраивать в лекции HTML/CSS/JS-контент, "
                    "который загружается через iframe. Это полезно для:\n"
                    "- Интерактивных симуляций\n"
                    "- Визуализаций данных\n"
                    "- Мини-игр и тренажеров\n\n"
                    "Файлы размещаются в папке `web-lection-files/` и доступны по пути "
                    "`/web-lection-files/<filename>`."
                ),
            }
        ],
        visible_group_ids=teacher_group_ids,
    ...
    lec2.public_id = pid2
    lec2.save()
    lessons.append(LessonRef(id=str(lec2.id), type="lecture", title=lec2.title, order=order))
    order += 1
...
def main():
    ...
    try:
        ...
        tracks.extend(create_web_lecture_track(users, groups))
